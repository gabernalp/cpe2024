...
<div class="container pt-5 pb-2">
	<div class="row pb-4">
		<div class="col-md-6">
			...
			<h1 class="pt-4">{{$subcategory->name ?? 'NombreSub'}}</h1>
        ...
        @foreach($resources as $key => $resource)
            ...
            <div class="col-md-2 mb-2" id="myResource{{$resource->id}}">
                <p>@if($resource->image_pdf)
                        <a target="_blank" href="{{ $resource->file->id }}"><img src="{{$resource->image_pdf->getUrl()}}" class="img-fluid" alt="" data-toggle="tooltip" data-html="true" title="<small>{{$resource->comments}}</small>">
                        </a>
                    @else
                    ...
                    @endif</p>
                <p class="text-center">@if(Auth::check())<i style="cursor: pointer" id="favorite{{$resource->id}}" class="fas fa-heart" data-toggle="tooltip" data-html="true" title="<small>Agregar a mi Biblioteca</small>"></i><input type="hidden" value="{{$resource->id}}" id="fav{{$resource->id}}"> <a target="_blank" href="{{ $resource->file->getUrl() }}">@endif<i class="fa fa-download"></i> <small>Descargar</small></a><br /><a target="_blank" href="{{ Storage::disk('public')->url('576/631669cfc37df_RETOUNICO1-TEMATICA4.pdf') }}"><small style="color: gray">{{ $resource->name }}</small></a><br />
                @if($resource->manual)
            ...
            <div class="col-md-2 mb-2" id="myResource{{$resource->id}}">
                ...
                <p class="text-center">@if(Auth::check())<i style="cursor: pointer" id="favorite{{$resource->id}}" class="fas fa-heart" data-toggle="tooltip" data-html="true" title="<small>Agregar a mi Biblioteca</small>"></i><input type="hidden" value="{{$resource->id}}" id="fav{{$resource->id}}">@endif <a  target="_blank" href="{{ $resource->link }}"><i class="fa fa-link"></i> <small>Ir al recurso</small></a></p>
...
<script type="text/javascript">
    $("#favorite{{$resource->id}}").click(function(){
        $.ajax({
            url: "{{ route('admin.resources.favorite') }}?resource_id={{$resource->id}}",
            method: 'GET',
...
<script type="text/javascript">
    $("#myResource{{$resource->id}}").click(function(){
        $.ajax({
            url: "{{ route('admin.resources.accessResource') }}?resource_id={{$resource->id}}",
            method: 'GET',
            success: function(data) {
