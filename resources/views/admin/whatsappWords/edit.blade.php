...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.whatsappWord.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.whatsapp-words.update", [$whatsappWord->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="word">{{ trans('cruds.whatsappWord.fields.word') }}</label>
                <input required class="form-control {{ $errors->has('word') ? 'is-invalid' : '' }}" type="text" name="word" id="word" value="{{ old('word', $whatsappWord->word) }}">
                @if($errors->has('word'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('object') ? 'is-invalid' : '' }}" type="text" name="object" id="object" value="{{ old('object', $whatsappWord->object) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('action') ? 'is-invalid' : '' }}" type="text" name="action" id="action" value="{{ old('action', $whatsappWord->action) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('link') ? 'is-invalid' : '' }}" type="text" name="link" id="link" value="{{ old('link', $whatsappWord->link) }}">
                @if($errors->has('link'))
                    <div class="invalid-feedback">
                        {{ $errors->first('link') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea required class="form-control {{ $errors->has('message') ? 'is-invalid' : '' }}" name="message" id="message">{{ old('message', $whatsappWord->message) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\WhatsappWord::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $whatsappWord->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('extra') ? 'is-invalid' : '' }}" type="text" name="extra" id="extra" value="{{ old('extra', $whatsappWord->extra) }}">
                @if($errors->has('extra'))
                    <div class="invalid-feedback">
                        {{ $errors->first('extra') }}
                    </div>
