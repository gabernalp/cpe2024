    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.profiles.update", [$profile->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input required class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $profile->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select required class="form-control select2 {{ $errors->has('entidad_asociada') ? 'is-invalid' : '' }}" name="entidad_asociada_id" id="entidad_asociada_id">
                    @foreach($entidad_asociadas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('entidad_asociada_id') ? old('entidad_asociada_id') : $profile->entidad_asociada->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select required class="form-control select2 {{ $errors->has('coursehooks') ? 'is-invalid' : '' }}" name="coursehooks[]" id="coursehooks" multiple>
                    @foreach($coursehooks as $id => $coursehook)
                        <option value="{{ $id }}" {{ (in_array($id, old('coursehooks', [])) || $profile->coursehooks->contains($id)) ? 'selected' : '' }}>{{ $coursehook }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('comments') ? 'is-invalid' : '' }}" name="comments" id="comments">{{ old('comments', $profile->comments) }}</textarea>
