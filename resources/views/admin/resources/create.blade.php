            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tag_categories') ? 'is-invalid' : '' }}" name="tag_categories[]" id="tag_categories" multiple>
                    @foreach($tag_categories as $id => $tag_category)
                        <option value="{{ $id }}" {{ in_array($id, old('tag_categories', [])) ? 'selected' : '' }}>{{ $tag_category }}</option>
                    @endforeach
                </select>
...
<script type="text/javascript">
    $("#resourcescategory_id").change(function(){
        $.ajax({
            url: "{{ route('admin.subcategory.get_by_category') }}?category_id=" + $(this).val(),
            method: 'GET',
            success: function(data) {
                $('#resource_subcategories').html(data.html);
            }
        });
    });
</script>
