        ...
        <form method="POST" action="{{ route("admin.courses.update", [$course->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select required class="form-control select2 {{ $errors->has('tematica_asociada') ? 'is-invalid' : '' }}" name="tematica_asociada_id" id="tematica_asociada_id">
                    @foreach($tematica_asociadas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('tematica_asociada_id') ? old('tematica_asociada_id') : $course->tematica_asociada->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select required class="form-control {{ $errors->has('tipo_ciclo') ? 'is-invalid' : '' }}" name="tipo_ciclo" id="tipo_ciclo">
                    ...
                    @foreach(App\Models\Course::TIPO_CICLO_SELECT as $key => $label)
                        <option  value="{{ $key }}" {{ old('tipo_ciclo', $course->tipo_ciclo) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('unico') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="unico" id="unico" value="1" {{ $course->unico || old('unico', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea required class="form-control {{ $errors->has('mensaje_cierre') ? 'is-invalid' : '' }}" name="mensaje_cierre" id="mensaje_cierre">{!! old('mensaje_cierre', $course->mensaje_cierre)!!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('additional_comments') ? 'is-invalid' : '' }}" name="additional_comments" id="additional_comments">{{ old('additional_comments', $course->additional_comments) }}</textarea>
