    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.challenges-users.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2" name="courseschedule_id" id="courseschedule_id">
                    @foreach($courseschedules as $courseschedule)
                        <option value="{{ $courseschedule->id }}" >{{ $courseschedule->start_date.' '.$courseschedule->course->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\ChallengesUser::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', 'No entregado') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
