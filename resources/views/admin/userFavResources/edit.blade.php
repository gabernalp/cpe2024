...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.userFavResource.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.user-fav-resources.update", [$userFavResource->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select required class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $userFavResource->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select required class="form-control select2 {{ $errors->has('resource') ? 'is-invalid' : '' }}" name="resource_id" id="resource_id">
                    @foreach($resources as $id => $entry)
                        <option value="{{ $id }}" {{ (old('resource_id') ? old('resource_id') : $userFavResource->resource->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
