@extends('layouts.admin')
@section('content')
@can('background_process_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.background-processes.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.backgroundProcess.title_singular') }}
            </a>
            
            @include('csvImport.modal', ['model' => 'BackgroundProcess', 'route' => 'admin.background-processes.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.backgroundProcess.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-BackgroundProcess">
            <thead>
                <tr>
                    <th width="10">

                    </th>
                    <th>
                        {{ trans('cruds.backgroundProcess.fields.id') }}
                    </th>
                    <th>
                        {{ trans('cruds.backgroundProcess.fields.name') }}
                    </th>
                    <th>
                        {{ trans('cruds.backgroundProcess.fields.description') }}
                    </th>
                    <th>
                        {{ trans('cruds.backgroundProcess.fields.file') }}
                    </th>
                    <th>
                        {{ trans('cruds.backgroundProcess.fields.images') }}
                    </th>
                    <th>
                        {{ trans('cruds.backgroundProcess.fields.link') }}
                    </th>
                    <th>
                        {{ trans('cruds.backgroundProcess.fields.especial') }}
                    </th>
                    <th>
                        {{ trans('cruds.backgroundProcess.fields.descripcion_especial') }}
                    </th>
                    <th>
                        {{ trans('cruds.backgroundProcess.fields.imagen_especial') }}
                    </th>
                    <th>
                        {{ trans('cruds.backgroundProcess.fields.comments') }}
                    </th>
                    <th>
                        &nbsp;
                    </th>
                </tr>
            </thead>
        </table>
    </div>
</div>



@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('background_process_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}';
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.background-processes.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).data(), function (entry) {
          return entry.id
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  let dtOverrideGlobals = {
    buttons: dtButtons,
    processing: true,
    serverSide: true,
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.background-processes.index') }}",
    columns: [
      { data: 'placeholder', name: 'placeholder' },
{ data: 'id', name: 'id' },
{ data: 'name', name: 'name' },
{ data: 'description', name: 'description' },
{ data: 'file', name: 'file', sortable: false, searchable: false },
{ data: 'images', name: 'images', sortable: false, searchable: false },
{ data: 'link', name: 'link' },
{ data: 'especial', name: 'especial' },
{ data: 'descripcion_especial', name: 'descripcion_especial' },
{ data: 'imagen_especial', name: 'imagen_especial', sortable: false, searchable: false },
{ data: 'comments', name: 'comments' },
{ data: 'actions', name: '{{ trans('global.actions') }}' }
    ],
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 10,
  };
  let table = $('.datatable-BackgroundProcess').DataTable(dtOverrideGlobals);
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  
});

</script>
@endsection