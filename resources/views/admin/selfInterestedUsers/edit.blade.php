    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.self-interested-users.update", [$selfInterestedUser->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="profile_id">{{ trans('cruds.selfInterestedUser.fields.profile') }}</label>
                <select required class="form-control select2 {{ $errors->has('profile') ? 'is-invalid' : '' }}" name="profile_id" id="profile_id">
                    @foreach($profiles as $id => $entry)
                        <option value="{{ $id }}" {{ (old('profile_id') ? old('profile_id') : $selfInterestedUser->profile->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.selfInterestedUser.fields.profile_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.selfInterestedUser.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $selfInterestedUser->name) }}" required>
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lastname') ? 'is-invalid' : '' }}" type="text" name="lastname" id="lastname" value="{{ old('lastname', $selfInterestedUser->lastname) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $selfInterestedUser->email) }}" required>
                @if($errors->has('email'))
                    <div class="invalid-feedback">
                        {{ $errors->first('email') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('documenttype') ? 'is-invalid' : '' }}" name="documenttype_id" id="documenttype_id" required>
                    @foreach($documenttypes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('documenttype_id') ? old('documenttype_id') : $selfInterestedUser->documenttype->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('document') ? 'is-invalid' : '' }}" type="number" name="document" id="document" value="{{ old('document', $selfInterestedUser->document) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('document_date') ? 'is-invalid' : '' }}" type="text" name="document_date" id="document_date" value="{{ old('document_date', $selfInterestedUser->document_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('phone') ? 'is-invalid' : '' }}" type="text" name="phone" id="phone" value="{{ old('phone', $selfInterestedUser->phone) }}">
                @if($errors->has('phone'))
                    <div class="invalid-feedback">
                        {{ $errors->first('phone') }}
                    </div>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\SelfInterestedUser::EDUCATION_BACKGROUND_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('education_background') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="education_background_{{ $key }}" name="education_background" value="{{ $key }}" {{ old('education_background', $selfInterestedUser->education_background) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('modality') ? 'is-invalid' : '' }}" name="modality" id="modality" required>
                    ...
                    @foreach(App\Models\SelfInterestedUser::MODALITY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('modality', $selfInterestedUser->modality) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('department') ? 'is-invalid' : '' }}" name="department_id" id="department_id">
                    @foreach($departments as $id => $entry)
                        <option value="{{ $id }}" {{ (old('department_id') ? old('department_id') : $selfInterestedUser->department->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('city') ? 'is-invalid' : '' }}" name="city_id" id="city_id">
                    @foreach($cities as $id => $entry)
                        <option value="{{ $id }}" {{ (old('city_id') ? old('city_id') : $selfInterestedUser->city->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('city'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('living_zone') ? 'is-invalid' : '' }}" name="living_zone" id="living_zone" required>
                    ...
                    @foreach(App\Models\SelfInterestedUser::LIVING_ZONE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('living_zone', $selfInterestedUser->living_zone) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('contacted') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="contacted" id="contacted" value="1" {{ $selfInterestedUser->contacted || old('contacted', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <label class="required" for="courseshooks">{{ trans('cruds.selfInterestedUser.fields.courseshooks') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select required class="form-control select2 {{ $errors->has('courseshooks') ? 'is-invalid' : '' }}" name="courseshooks[]" id="courseshooks" multiple>
                    @foreach($courseshooks as $id => $courseshook)
                        <option value="{{ $id }}" {{ (in_array($id, old('courseshooks', [])) || $selfInterestedUser->courseshooks->contains($id)) ? 'selected' : '' }}>{{ $courseshook }}</option>
                    @endforeach
                </select>
