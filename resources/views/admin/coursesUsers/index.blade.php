@extends('layouts.admin')
@section('content')
@can('courses_user_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.courses-users.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.coursesUser.title_singular') }}
            </a>
            
            @include('csvImport.modal', ['model' => 'CoursesUser', 'route' => 'admin.courses-users.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.coursesUser.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-CoursesUser">
            <thead>
                <tr>
                    <th width="10">

                    </th>
                    <th>
                        {{ trans('cruds.coursesUser.fields.id') }}
                    </th>
                    <th>
                        {{ trans('cruds.coursesUser.fields.user') }}
                    </th>
                    <th>
                        Telefono
                    </th>
                    <th>
                        {{ trans('cruds.user.fields.document') }}
                    </th>
                    <th>
                        {{ trans('cruds.coursesUser.fields.course_schedule') }}
                    </th>
                    <th>
                        {{ trans('cruds.coursesUser.fields.course_name') }}
                    </th>
                    <th>
                        {{ trans('cruds.coursesUser.fields.whatsapp_user') }}
                    </th>
                    <th>
                        {{ trans('cruds.coursesUser.fields.actual_challenge') }}
                    </th>
                    <th>
                        {{ trans('cruds.coursesUser.fields.alert_messages') }}
                    </th>
                    <th>
                        {{ trans('cruds.coursesUser.fields.status') }}
                    </th>
                    <th>&nbsp;
                        
                    </th>
                </tr>
            </thead>
        </table>
    </div>
</div>



@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('courses_user_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}';
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.courses-users.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).data(), function (entry) {
          return entry.id
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  let dtOverrideGlobals = {
    buttons: dtButtons,
    processing: true,
    serverSide: true,
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.courses-users.index') }}",
    columns: [
      { data: 'placeholder', name: 'placeholder' },
{ data: 'id', name: 'id' },
{ data: 'user_name', name: 'user.name' },
{ data: 'user.phone', name: 'user.phone' },
{ data: 'user.document', name: 'user.document' },
{ data: 'course_schedule_start_date', name: 'course_schedule.start_date' },
{ data: 'course_name', name: 'course_name' },
{ data: 'whatsapp_user', name: 'whatsapp_user' },
{ data: 'actual_challenge', name: 'actual_challenge' },
{ data: 'alert_messages', name: 'alert_messages' },
{ data: 'status', name: 'status' },
{ data: 'actions', name: '{{ trans('global.actions') }}' }
    ],
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 25,
    lengthMenu: [ [25, 50, 100, 500, 1000, 2000], [25, 50, 100, 500, 1000, 2000] ],
  };
  let table = $('.datatable-CoursesUser').DataTable(dtOverrideGlobals);
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  
});

</script>
@endsection