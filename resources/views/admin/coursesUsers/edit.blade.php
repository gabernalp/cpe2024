        ...
        <form method="POST" action="{{ route("admin.courses-users.update", [$coursesUser->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select required class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $coursesUser->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select required class="form-control select2 {{ $errors->has('course_schedule') ? 'is-invalid' : '' }}" name="course_schedule_id" id="course_schedule_id">
                    @foreach($course_schedules as $id => $entry)
                        <option value="{{ $id }}" {{ (old('course_schedule_id') ? old('course_schedule_id') : $coursesUser->course_schedule->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('whatsapp_user') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="whatsapp_user" id="whatsapp_user" value="1" {{ $coursesUser->whatsapp_user || old('whatsapp_user', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input required class="form-control {{ $errors->has('actual_challenge') ? 'is-invalid' : '' }}" type="number" name="actual_challenge" id="actual_challenge" value="{{ old('actual_challenge', $coursesUser->actual_challenge) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select required class="form-control {{ $errors->has('alert_messages') ? 'is-invalid' : '' }}" name="alert_messages" id="alert_messages">
                    ...
                    @foreach(App\Models\CoursesUser::ALERT_MESSAGES_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('alert_messages', $coursesUser->alert_messages) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('manual_feedback') ? 'is-invalid' : '' }}" name="manual_feedback" id="manual_feedback">{{ old('manual_feedback', $coursesUser->manual_feedback) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('additional_link') ? 'is-invalid' : '' }}" type="text" name="additional_link" id="additional_link" value="{{ old('additional_link', $coursesUser->additional_link) }}">
