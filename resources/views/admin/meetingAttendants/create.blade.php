    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.meeting-attendants.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select required class="form-control select2 {{ $errors->has('meeting') ? 'is-invalid' : '' }}" name="meeting_id" id="meeting_id">
                    @foreach($meetings as $id => $entry)
                        <option value="{{ $id }}" {{ old('meeting_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
